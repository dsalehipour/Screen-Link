...
/// Maps browser `KeyboardEvent.code` values to macOS ANSI virtual keycodes.
/// These keycodes are a fixed hardware mapping, not a layout mapping, so they are stable constants.
/// Split into groups because a single literal this large blows the type checker's budget.
enum KeyMap {
    ...
    private static let digitsAndPunctuation: [String: CGKeyCode] = [
    ...
    private static let table: [String: CGKeyCode] = {
        var all = letters
        for group in [digitsAndPunctuation, controls, navigation, functionKeys, numpad] {
            all.merge(group) { current, _ in current }
        }
        return all
    }()
}
